...
class Trial extends Item
{
    ...
    public static function insertAllowed($idExoDevoir,$idUser)
    {
        ...
        try {
            ...
            $stmt = $pdo->prepare(<<<SQL
                SELECT devoirs.id
                FROM {$prefix}devoirs AS devoirs
                JOIN {$prefix}users AS users ON users.idClasse = devoirs.idClasse
                JOIN {$prefix}exodevoirs AS exodevoirs ON exodevoirs.idDevoir = devoirs.id
    ...
    protected function onUpdateSuccess()
    {
        ...
        try {
            ...
            $prefix = PREFIX_BDD;
            $stmt = $pdo->prepare(<<<SQL
                UPDATE {$prefix}noteexos
                SET note = GREATEST(note,$score)
                WHERE idExoDevoir = :idExoDevoir AND idUser = :idUser
            SQL
            );
            $stmt->bindValue(":idExoDevoir", $idExoDevoir, PDO::PARAM_INT);
            $stmt->bindValue(":idUser", $idUser, PDO::PARAM_INT);
    ...
    protected function onInsertSuccess()
    {
        ...
        try {
            ...
            $prefix = PREFIX_BDD;
            $stmt = $pdo->prepare(<<<SQL
                INSERT INTO {$prefix}noteexos (idExoDevoir, idUser, note, trials)
