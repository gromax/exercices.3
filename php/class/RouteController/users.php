...
class users
{
  ...
  public function insert()
  {
    ...
    if (!$uLog->connexionOk())
    {
      ...
      // en vérifiant les paramètres
      $data['rank'] = User::RANK_ELEVE;
      $user=new User($data);
      $validation = $user->insertion_validation();
      
      if (is_array($validation) || !$classe->testPwd($pwdClasse))
      {
        // au moins une erreur
        if (!is_array($validation)) $validation = array();
        if (!$classe->testPwd($pwdClasse)) $validation['classeMdp']=["Mot de passe invalide."];
