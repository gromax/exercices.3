...
const FormView = View.extend({
    ...
    showErrors(errors:Record<string,string|Array<string>>):void {
        let $view = this.$el
        this.clearForm()

        $view.find("input").each(
            function(index:number, item:HTMLElement) {
        ...
        Object.entries(errors).forEach(([key, value]) => {
            ...
            if (Array.isArray(value)){
                let reduceFct = function(m:string, v:string, i:number) { return `${m}<p>${v}</p>` }
